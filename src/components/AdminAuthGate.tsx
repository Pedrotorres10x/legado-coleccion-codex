...
interface AdminAuthGateProps {
  children: (auth: ReturnType<typeof useAdminAuth>) => ReactNode;
}
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          ...
          <CardContent className="space-y-4">
            ...
            <Button onClick={auth.handleLogin} disabled={auth.loginLoading} className="w-full">
              <GoogleIcon />
              {auth.loginLoading ? "Accediendo..." : "Iniciar sesión con Google"}
            </Button>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="w-full max-w-md text-center">
          <CardContent className="pt-6 space-y-4">
            ...
            <Button variant="outline" onClick={auth.handleLogout}>Cerrar sesión</Button>
          </CardContent>
        </Card>
      </div>
    );
